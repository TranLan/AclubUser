...
public class RestClient {
	...
	static {
		_asyncClient.setTimeout(50 * 1000);
	}
	...
	private static Header[] buildUsedHeader(Context context) {
		Header[] header = {
				new BasicHeader("Content-Type",
						"application/json;charset=UTF-8"),
				new BasicHeader("application_token",
						"b0bb724a0c6a6654742a6b667de939998ef70c6b2569516054a4ee"),
				new BasicHeader("phone", SpManager.getInstances(context)
						.getString(SPConstants.NUM_PHONE, "")),
				new BasicHeader("TOKEN", SpManager.getInstances(context)
						.getString(SPConstants.NUM_TOKEN, "")) };
		return header;
	}

	private static Header[] buildHeaderForUploadAvatar(Context context) {
		Header[] header = {
	...
	public static void get(Context context, String url, RequestParams params,
			AsyncHttpResponseHandler responseHandler, boolean isUseHeader) {
		DLog.d(TAG, url);
		DLog.d(TAG, "param: " + params);
		Header[] header;
		if (isUseHeader) {
			header = buildUsedHeader(context);
		} else {
			header = buildHeader();
		}
	...
	public static void post(Context context, String url, String jsonParams,
			AsyncHttpResponseHandler responseHandler, boolean isNewHeader) {
		DLog.d(TAG, url);
		Header[] header = null;
		if (isNewHeader) {
			header = buildUsedHeader(context);
	...
	public static void put(Context context, String url, RequestParams params,
		...
		Header[] header = buildHeaderForUploadAvatar(context);
		// if (isNewHeader) {
		// header = buildUsedHeader(context);
		// } else {
		// header = buildHeader();
		...
		// _asyncClient.put
		//
		// _asyncClient.put(context, getAbsoluteUrl(url), header, params,
		// responseHandler);
		// _asyncClient.put(context, getAbsoluteUrl(url), header, params, "",
		// responseHandler);

		// _asyncClient.put(context, url, headers, entity, contentType,
		// responseHandler)
	}
}
